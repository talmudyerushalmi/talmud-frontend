...
export function getSugiaLines(currentMishna: iMishna, line: iLine) {  
  ...
  const lineNextSugia = lines.find(
    (mishnaLine) =>
      mishnaLine.sugiaName &&
      parseInt(mishnaLine.lineNumber!) > parseInt(line.lineNumber!)
  );

  let lineSugiaEnd;
  if (lineNextSugia) {
    lineSugiaEnd = lines.find(
      (l) => parseInt(l.lineNumber!) === parseInt(lineNextSugia.lineNumber!) - 1
    );
  } else {
