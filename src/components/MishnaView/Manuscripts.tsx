...
const Manuscripts = (props: Props) => {
  ...
  const handleClickOpen = () => () => {
    if (manuscripts.length === 0) {
      RelatedService.getRelated(tractate, chapter).then((res) => {
        ...
        const m = res.manuscripts.sort(compare).map((manuscript) => {
          return {
            ...manuscript,
  ...
  return (
    <ImageList cols={1} sx={{ width: 300, height: 450 }}>
      {manuscripts.map((manuscript) => (
        <a target="_blank" rel="noreferrer" href={manuscript.imageurl}>
          <ImageListItem key={manuscript.imageurl}>
            <img
              ...
              loading="lazy"
            />
            <ImageListItemBar title={manuscript.slug} position="below" />
          </ImageListItem>
        </a>
      ))}
    </ImageList>
